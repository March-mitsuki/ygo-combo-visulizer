...
import { toolBarEvents } from "../tool_bar/events";
    ...
    if (!stepList.list) {
      const mockNode = makeNode(
        ...
        0,
        {
          ...
          y: ORIGIN_POINT.y + nodeBoxHeight * stepList.idx,
          ...commonNodeOpt,
        },
      );
...
export const StepRenderer: React.FC<{
  ...
  useEffect(() => {
    ...
    contentsLayer.position({
      x: -ORIGIN_POINT.x + 200,
      y: -ORIGIN_POINT.y + 200,
    });
    ...
    toolBarEvents.on("back-to-origin", () => {
      if (!contentsLayer) return;
      contentsLayer.position({
        x: -ORIGIN_POINT.x + 200,
        y: -ORIGIN_POINT.y + 200,
      });
